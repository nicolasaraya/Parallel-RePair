...
void Heap::verify(long long int pos){
    long long int posUp=(pos)/2;
    if(posUp<1) {
        //cout << "Error pos up: " <<  posUp << endl;
        return;
    }
    else{
...
void Heap::verifyDown(long long int pos){
    long long int l = (2*pos);
    long long int r = (2*pos)+1;
    if(l > size()) return;
    if(l == size()){
